...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        for(int column=0; column<=2; column++){
            if(grid[column][0]==grid[column][1] && grid[column][1]==grid[column][2] && (grid[column][0]=='x' || grid[column][0]=='o')){
                winner = grid[column][0];
                checkState = false;
                break;
            }
        }
        ...
        if(checkState){
            for(int row=0; row<=2;row++){
                if(grid[0][row]==grid[1][row] && grid[1][row]==grid[2][row] && (grid[0][row]=='x' || grid[0][row]=='o')){
                    winner = grid[0][row];
                    checkState = false;
                    break;
                }
